...
import scorex.core.consensus.History.{HistoryComparisonResult, Nonsense, Unknown, Younger}
import scorex.core.consensus.{History, HistoryReader, SyncInfo}
import scorex.core.mainviews.{NodeViewHolder, PersistentNodeViewModifier}
import scorex.core.mainviews.NodeViewHolder.ReceivableMessages.{CompareViews, GetNodeViewChanges, ModifiersFromRemote, Subscribe}
...
import scorex.core.transaction.wallet.{Vault, Wallet}
import scorex.core.transaction.{MempoolReader, Transaction}
import scorex.core.utils.{NetworkTimeProvider, ScorexLogging}
import scorex.core.{mainviews, _}
...
PMOD <: mainviews.PersistentNodeViewModifier,
...
MR <: MempoolReader[TX]](networkControllerRef: ActorRef,
  ...
  protected def broadcastModifierInv[M <: mainviews.NodeViewModifier](m: M): Unit = {
    val msg = Message(invSpec, Right(m.modifierTypeId -> Seq(m.id)), None)
    networkControllerRef ! SendToNetwork(msg, Broadcast)
  }

  protected def broadcastBlock[M <: mainviews.NodeViewModifier](m: M): Unit = {
  ...
  protected def viewHolderEvents: Receive = {
    ...
    //TODO replace with ^
    case BlockComparingModifier(mod) =>
      log.debug(s"block : $mod")
      broadcastBlock(mod)
  ...
  protected def getLocalSyncInfo: Receive = {
    case SendLocalSyncInfo =>
      ...
      if (statusTracker.elapsedTimeSinceLastSync() < (networkSettings.syncInterval.toMillis / 2)) {
        ...
        historyReaderOpt.foreach(r => Try{sendSync(r.syncInfo)})
      }
  }
  ...
  protected def modifiersReq: Receive = {
    case DataFromPeer(spec, invData: InvData@unchecked, remote)
      ...
      readersOpt.foreach { readers =>
        val objs: Seq[mainviews.NodeViewModifier] = invData._1 match {
  ...
  protected def requestFromLocal: Receive = {
    case RequestFromLocal(peer, modifierTypeId, modifierIds) =>
      log.debug(s"requestFromLocal: modifiersIds len: ${modifierIds.length}")
  ...
  protected def responseFromLocal: Receive = {
    case ResponseFromLocal(peer, _, modifiers: Seq[mainviews.NodeViewModifier]) =>
...
object NodeViewSynchronizer {
  object ReceivableMessages {
    ...
    case class ResponseFromLocal[M <: mainviews.NodeViewModifier](source: ConnectedPeer, modifierTypeId: ModifierTypeId, localObjects: Seq[M])
    ...
    case class ChangedHistory[HR <: HistoryReader[_ <: mainviews.PersistentNodeViewModifier, _ <: SyncInfo]](reader: HR) extends NodeViewChange
    ...
    case class ChangedVault[V<: Vault[_, _, _, _]](vault:V) extends NodeViewChange
    ...
    case class SyntacticallyFailedModification[PMOD <: mainviews.PersistentNodeViewModifier](modifier: PMOD, error: Throwable) extends ModificationOutcome
    case class SemanticallyFailedModification[PMOD <: mainviews.PersistentNodeViewModifier](modifier: PMOD, error: Throwable) extends ModificationOutcome
    case class SyntacticallySuccessfulModifier[PMOD <: mainviews.PersistentNodeViewModifier](modifier: PMOD) extends ModificationOutcome
    case class SemanticallySuccessfulModifier[PMOD <: mainviews.PersistentNodeViewModifier](modifier: PMOD) extends ModificationOutcome
    //TODO replace with ^
    case class BlockComparingModifier[PMOD <: mainviews.PersistentNodeViewModifier](modifier: PMOD) extends ModificationOutcome
  }
}
