...
import scorex.core.LocalInterface.ReceivableMessages.{ChangedState, NewOpenSurface, RollbackFailed, StartingPersistentModifierApplication}
import scorex.core.LocallyGeneratedModifiersMessages.ReceivableMessages._
import scorex.core.{ModifierId, ModifierTypeId, VersionTag}
import scorex.core.consensus.History.ProgressInfo
import scorex.core.consensus.{History, HistoryReader, SyncInfo}
import scorex.core.mainviews.NodeViewHolder.{CurrentView, DownloadRequest, EventType}
import scorex.core.mainviews.NodeViewHolder.ReceivableMessages._
...
trait NodeViewHolder[P <: Proposition, TX <: Transaction[P], PMOD <: PersistentNodeViewModifier]
  ...
  val modifiersCache = mutable.Map[MapKey, PMOD]()
  ...
  protected def txModify(tx: TX): Unit = {
    ...
    val errorOpt: Option[Throwable] = None/*minimalState() match {
  ...
  protected def updateNodeView(updatedHistory: Option[HIS] = None,
    ...
    log.debug(s"Trigger : updateNodeView : $updatedHistory, $updatedState, $updatedVault, $updatedMempool")
    ...
    if (updatedVault.nonEmpty) {
      notifySubscribers[ChangedVault[VL]](EventType.VaultChanged, ChangedVault(updatedVault.get))
    }
  ...
  protected def updateMemPool(progressInfo: History.ProgressInfo[PMOD], memPool: MP, state: MS): MP = {
    ...
    log.debug(s"appliedTxs size: ${appliedTxs.length}")
          ...
          if (progressInfo.toApply.nonEmpty) {
            ...
            newStateTry match {
              ...
              case Failure(e) =>
                ...
                updateNodeView(updatedHistory = Some(newHistory), updatedVault = vault())
                notifySubscribers(EventType.SemanticallyFailedPersistentModifier, SemanticallyFailedModification(pmod, e))
            }
          } else {
            requestDownloads(progressInfo)
            updateNodeView(updatedHistory = Some(historyBeforeStUpdate), updatedVault = vault())
          }
        case Failure(e) =>
    ...
    case ModifiersFromRemote(remote, modifierTypeId, remoteObjects) =>
      modifierSerializers.get(modifierTypeId) foreach { companion =>
        remoteObjects.flatMap(r => companion.parseBytes(r).toOption).foreach {
          case block: PMOD@unchecked =>
            if (history().contains(block) || modifiersCache.contains(key(block.id))) {
              log.warn(s"Received modifier ${block.encodedId} that is already in history")
            } else {
              modifiersCache.put(key(block.id), block)
            }
        }
  ...
  protected def processLocallyGeneratedModifiers: Receive = {
    case lm: LocallyGeneratedModifier[PMOD] =>
      ...
      //pmodModify(lm.pmod)
      notifySubscribers (EventType.MinedBlock, BlockComparingModifier(lm.pmod))
  }
  ...
  protected def getNodeViewChanges: Receive = {
    case GetNodeViewChanges(history, state, vault, mempool) =>
      ...
      if (vault) sender() ! ChangedVault(nodeView._3.get)
...
object NodeViewHolder {
  ...
  object ReceivableMessages {
    ...
    case class RemoteBestBlockModifier[PMOD <: PersistentNodeViewModifier](pmod: PMOD)
  ...
  object EventType extends Enumeration {
    ...
    val MinedBlock: EventType.Value = Value(15)
  }
