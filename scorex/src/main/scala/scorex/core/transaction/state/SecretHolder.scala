...
trait SecretCompanion[S <: Secret] {
  ...
  def generateKeys(randomSeed: Array[Byte], order:Int): (S, PK)
}

case class PrivateKey25519(privKeyBytes: PrivateKey, publicKeyBytes: PublicKey, order:Int) extends Secret {
...
object PrivateKey25519Serializer extends Serializer[PrivateKey25519] {
  override def toBytes(obj: PrivateKey25519): Array[Byte] = Bytes.concat(obj.privKeyBytes, obj.publicKeyBytes, Ints.toByteArray(obj.order))

  private def orderFromBytes (bytes:Array[Byte]):Int = {
    if (bytes.length>0) Ints.fromByteArray(bytes) else 0
  }
  override def parseBytes(bytes: Array[Byte]): Try[PrivateKey25519] = Try {
    PrivateKey25519(PrivateKey @@ bytes.slice(0, 32), PublicKey @@ bytes.slice(32, 64), orderFromBytes(bytes.slice(64, 68)))
  }
}
...
object PrivateKey25519Companion extends SecretCompanion[PrivateKey25519] {
  ...
  override def generateKeys(randomSeed: Array[Byte], order:Int): (PrivateKey25519, PublicKey25519Proposition) = {
    val pair = Curve25519.createKeyPair(randomSeed)
    val secret: PrivateKey25519 = PrivateKey25519(pair._1, pair._2, order)
